    ...
    <div className="bg-gray-900 rounded-2xl overflow-hidden border border-gray-800 hover:border-gray-600 transition group">
      ...
      <div className="p-4">
        ...
        <button onClick={() => onRemove(item.movieId)}
          className="w-full bg-red-600/20 hover:bg-red-600/40 border border-red-500/30 text-red-400 text-xs font-semibold py-2 rounded-lg transition">
          ❤️ Favorited
        </button>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      <div className="max-w-7xl mx-auto pt-24 px-4 pb-16">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white">❤️ My Favorites</h1>
          <p className="text-gray-400 text-sm mt-1">
            {items.length > 0 ? `${items.length} saved` : "No favorites yet"}
          </p>
        </div>
        ...
        {!loading && items.length === 0 && (
          <div className="text-center mt-24">
            ...
            <p className="text-white text-lg font-semibold">No favorites yet</p>
            <p className="text-gray-500 text-sm mt-1">Hit the ❤️ button on any movie</p>
          </div>
        )}
        {items.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5">
            {items.map((item) => (
              <FavoriteCard key={item.movieId} item={item} onRemove={(id) => dispatch(removeFromFavorites(id))} />
            ))}
          </div>
        )}
      </div>
