...
export default function Movies() {
  ...
  const hasMore = selectedGenre ? genrePage < genreTotal : moviesPage < moviesTotalPages;
  ...
  const loadMore = useCallback(() => {
    if (selectedGenre) {
      if (!genreLoading && genrePage < genreTotal) loadGenre(selectedGenre, genrePage + 1);
    } else {
      if (!moviesLoading && moviesPage < moviesTotalPages) dispatch(fetchMoviesPage(moviesPage + 1));
    }
    ...
    <div className="min-h-screen bg-gray-950">
      <div className="max-w-7xl mx-auto pt-24 px-4 pb-16">
        <div className="mb-6">
          ...
          <p className="text-gray-400 text-sm mt-1">{displayList.length} movies loaded</p>
        </div>
        <GenreFilter genres={genres} selectedId={selectedGenre} onSelect={handleGenreSelect} />
        {error && <p className="text-red-400 text-sm mb-6">{error}</p>}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5">
          {displayList.map((item) => (
            <MovieCard key={item.id} item={{ ...item, media_type: "movie" }} />
          ))}
          {isLoading && Array(10).fill(0).map((_, i) => <SkeletonCard key={`sk-${i}`} />)}
        </div>
        {hasMore && <div ref={sentinelRef} className="h-10 mt-6" />}
        {!hasMore && displayList.length > 0 && (
          <p className="text-center text-gray-600 text-sm mt-10">You've reached the end 🎬</p>
        )}
      </div>
    </div>
  );
